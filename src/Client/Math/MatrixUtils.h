...
class MatrixUtils{
public:
    constexpr float ToRadians(float degrees){return degrees * 0.0174533f;}
    ...
    static void CreatePerspectiveProjection(float* dst, float fovInDegrees, float aspectRatio, float pNear, float pFar);
...
#pragma region Templates
    ...
    static void TranslateMat4x4(float* dst, const Vec3<TYPE>& vector) noexcept{
        AddMat4DataBy(dst, 0, 3, vector.x);
        AddMat4DataBy(dst, 1, 3, vector.y);
        AddMat4DataBy(dst, 2, 3, vector.z);
    }
#pragma endregion
private:
};